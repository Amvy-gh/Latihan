import matplotlib.pyplot as plt
import time

# Daftar slot waktu tersedia
time_slots = [
    "Senin 08:00", "Senin 10:00", "Selasa 08:00", "Selasa 10:00",
    "Rabu 08:00", "Rabu 10:00"
]

# Daftar tugas/aktivitas
tasks = [
    "Matematika", "Fisika", "Bahasa", "Kimia"
]

solutions = []

def is_valid(schedule, task, slot):
    return slot not in schedule.values()

# Ini buat simulasi step-by-step
def backtrack(schedule, task_idx):
    if task_idx == len(tasks):
        print(f"[SUKSES] Jadwal lengkap: {schedule}\n")
        solutions.append(schedule.copy())
        return

    task = tasks[task_idx]
    print(f"Mencoba menjadwalkan '{task}'...")

    for slot in time_slots:
        print(f"  Coba slot '{slot}' untuk '{task}'...")
        if is_valid(schedule, task, slot):
            schedule[task] = slot
            print(f"    [OKE] '{task}' dijadwalkan di '{slot}'. Lanjut tugas berikutnya.\n")
            backtrack(schedule, task_idx + 1)
            print(f"    [BACKTRACK] Lepas '{task}' dari slot '{slot}'. Coba pilihan lain...\n")
            del schedule[task]
        else:
            print(f"    [GAGAL] Slot '{slot}' sudah dipakai, cari slot lain...\n")
        time.sleep(0.5)  # Biar kelihatan stepnya pelan-pelan

# Mulai simulasi
backtrack({}, 0)

# Ambil solusi pertama
selected_schedule = solutions[0]

# Visualisasi Gantt Chart
slot_mapping = {
    "Senin 08:00": 0,
    "Senin 10:00": 1,
    "Selasa 08:00": 2,
    "Selasa 10:00": 3,
    "Rabu 08:00": 4,
    "Rabu 10:00": 5
}

# Warna berbeda untuk tiap tugas
task_colors = {
    "Matematika": "tab:red",
    "Fisika": "tab:green",
    "Bahasa": "tab:orange",
    "Kimia": "tab:purple"
}

def visualize_gantt(schedule):
    fig, ax = plt.subplots(figsize=(10, 5))

    yticks = []
    ylabels = []
    for idx, (task, slot) in enumerate(schedule.items()):
        start = slot_mapping[slot]
        ax.broken_barh([(start * 2, 1.8)], (idx * 5, 4), facecolors=task_colors[task])
        yticks.append(idx * 5 + 2)
        ylabels.append(task)

    ax.set_yticks(yticks)
    ax.set_yticklabels(ylabels)
    ax.set_xlabel('Waktu (slot)')
    ax.set_title('Penjadwalan Otomatis (Backtracking + Gantt Chart)')
    ax.set_xticks([i*2 for i in range(len(slot_mapping))])
    ax.set_xticklabels(list(slot_mapping.keys()), rotation=45, ha='right')
    plt.grid(True)
    plt.tight_layout()
    plt.show()

# Tampilkan Gantt Chart
visualize_gantt(selected_schedule)
